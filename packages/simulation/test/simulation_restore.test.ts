...
describe("simulation_restore", () => {
  it("restores from persisted state without sharing references", () => {
    const simulation = new Simulation(7, 12, 12, content, [{ id: "agent-1", name: "Agent 1" }]);
    const state = simulation.getState();
    state.tick = 123;
    state.agents[0]!.x = 4;
    state.agents[0]!.y = 6;
    state.agents[0]!.stamina = 77;
    state.agents[0]!.inventory = { campfire: 2 };
    ...
    state.agents[0]!.x = 0;
    expect(restored.getState().agents[0]?.x).toBe(4);

    const result = restored.applyAction("agent-1", { type: "place", prefabId: "campfire", x: 5, y: 6 });
